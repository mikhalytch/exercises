...
--  in this function.
evenLists :: [[a]] -> [[a]]
evenLists lol = map fst (filter (even . snd) (zipOn length lol))
  where
    zipOn :: (a -> b) -> [a] -> [(a, b)]
    zipOn f a = zip a (map f a)
...
data Knight = Knight {knightHealth :: Health, knightAttack :: Attack, knightEndurance :: Endurance}

newtype NonZeroGold = NonZeroGold Natural

newtype Treasure a = Maybe a

data Chest a = Chest {gold :: NonZeroGold, treasure :: Treasure a}

data DragonType = Red | Black | Green deriving (Show)

experienceForDragon :: DragonType -> Experience
experienceForDragon Red = Experience 150
experienceForDragon Black = Experience 200
experienceForDragon Green = Experience 250

data Dragon a = Dragon {dragonHealth :: Health, dragonFirePower :: Attack, dragonType::DragonType, chest :: Chest a}

newtype Reward a = Reward(NonZeroGold, Maybe (Treasure a))

rewardForDragon :: DragonType -> Chest a -> Reward a
rewardForDragon Green (Chest gold _) = Reward(gold, Nothing)
rewardForDragon Black (Chest gold a) = Reward(gold, Just a)
rewardForDragon Red (Chest gold a) = Reward(gold, Just a)

atk :: Health -> Attack -> Health
atk (Health h) (Attack a) = Health(h - a)

data FightResult a
  = KnightRuns
  | KnightDies
  | KnightWins(Experience, Reward a)

knightShouldDie :: Knight -> Bool
knightShouldDie (Knight (Health kh) _ _) = kh <= 0

dragonShouldDie :: Dragon a -> Bool
dragonShouldDie (Dragon (Health dh) _ _ _) = dh <= 0

knightShouldRun :: Knight -> Bool
knightShouldRun (Knight _ _ (Endurance ke)) = ke <= 0

decEndurance :: Endurance -> Endurance
decEndurance (Endurance e) = Endurance( e -1)

dragonFight :: Dragon a -> Knight -> FightResult a
dragonFight = strike 0
  where
    strike :: Int -> Dragon a -> Knight -> FightResult a
    strike step dragon knight
      | knightShouldDie knight = KnightDies
      | dragonShouldDie dragon = KnightWins(experienceForDragon (dragonType dragon), rewardForDragon (dragonType dragon) (chest dragon))
      | knightShouldRun knight = KnightRuns
      | otherwise =
        let dragonShouldFireThisStep = mod step 10 == 0
        in if dragonShouldFireThisStep
          then strike (step+1) dragon{dragonHealth=atk (dragonHealth dragon) (knightAttack knight)} knight
          else strike (step+1) dragon{dragonHealth=atk (dragonHealth dragon) (knightAttack knight)} knight{knightEndurance=decEndurance (knightEndurance knight)}
...
isIncreasing [] = True
-- isIncreasing (x:y:xs) = if x > y then False else isIncreasing (y:xs)
-- isIncreasing (x:y:xs) = not (x > y) && isIncreasing (y:xs)
...
merge = go []
  where
    ...
    go res [] bs = res ++ bs
    go res as [] = res ++ as
    go res (a : as) (b : bs) = if a > b then go (res ++ [b]) (a : as) bs else go (res ++ [a]) as (b : bs)
